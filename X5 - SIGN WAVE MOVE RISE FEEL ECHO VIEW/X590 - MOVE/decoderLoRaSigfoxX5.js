...
function decodeUplink(input) 
{
    ...
    function typeOfProduct(octetTypeProduit)
    {
        ...
        if(octetTypeProduit==0xC9){return "Echo LoRa"}
        if(octetTypeProduit==0xCA){return "View LoRa"}
    }    
    ...
    function typeOfMessage(octetTypeMessage)
    {
        ...
        if(octetTypeMessage==0x11){return "Sigfox/ LoRa US Periodic Data"}
        if(octetTypeMessage==0x15){return "Sigfox/ LoRa US Product Status"}
        if(octetTypeMessage==0x16){return "Sigfox/ LoRa US Product Configuration N1"}
        if(octetTypeMessage==0x17){return "Sigfox/ LoRa US Product Configuration N2"}
    }
    ...
    function airDriveIndex(octetAirDrive)
    {
        if(octetAirDrive===512){return "Error"}
        if(octetAirDrive>=511){return "Deconnected sensor"}
        if(octetAirDrive>=510){return "Desactivated sensor"}
        else{return {"value":octetAirDrive}};
    }
    ...
    */
    ...
    function periodicDataOutput(stringHex)
    {
        ...
        var data_air_drive =(parseInt(stringHex.substring(27,30),16) >> 2) & 0x1FF;
        ...
        "airDrive": airDriveIndex(data_air_drive)
        };
        return data;
    }
    ...
    function productStatusDataOutput(stringHex)
    {
        ...
        var data_hw_status_cov = (parseInt(stringHex.substring(13,15),16)>>3) & 0x07;
        ...
        "statusTemperature":sensorStatusArgument(data_hw_status_temperature),
        "statusCo2":sensorStatusArgument(data_hw_status_co2),
        "statusVoc":sensorStatusArgument(data_hw_status_cov),
        "statusMotion":sensorStatusArgument(data_hw_status_pir),
        "statusMicrophone":sensorStatusArgument(data_hw_status_microphone),
        "statusLuminosity":sensorStatusArgument(data_hw_status_luminosity),
    ...
    function productConfigurationDataOutput(stringHex)
    {
        ...
        "dateHours": {"value":data_date_produit_heure,"unit":"hours"},
        "dateMinutes": {"value":data_date_produit_minute,"unit":"minutes"}
        };
    ...
    function sigfoxProductStatusDataOutput(stringHex)
    {
        ...
        "statusLuminosity":sensorStatusArgument(data_hw_status_luminosity),
        "statusSdCard":sdStatusArgument(data_hw_status_SD),
        "timeActivation":productActivationTimeCounterSF(data_activation_time),
        "timeCo2LastCalibration":{"value":data_co2_last_manual_calibration_time,"unit":"weeks"},
        "statusAntiTear": antiTearArgument(data_hw_status_anti_tear)
        }
        return data
    ...
    function sigfoxProductConfigurationN1DataOutput(stringHex)
    {
        ...
        data={
            ...
            "typeOfMessage": typeOfMessage(octetTypeMessage),
            "sourceReconfiguration": reconfigurationSource(data_config_source),
            "statusReconfiguration": reconfigurationState(data_config_status),
            "periodMeasure": period(data_periode_mesure),
            "enableCo2":sensorActivation(data_sensor_on_off_co2),
            "enableVoc":sensorActivation(data_sensor_on_off_cov),
            "enableVoc":sensorActivation(data_sensor_on_off_pir),
            "enableMicrophone":sensorActivation(data_sensor_on_off_microphone),
            "enableMicrophone":sdActivation(data_sd_storage_on_off),
            "enableAutomaticCo2Calibration": calibrationActivation(data_co2_auto_calibration),
            "thresholdCo2Medium":co2Threshold(data_co2_medium_level),
            "thresholdCo2Medium":co2Threshold(data_co2_high_level),
            "enableLedCo2":active(data_led_on_off),
            "enableLedMediumLevel":active(data_led_orange_on_off),
            "enableBuzzer":active(data_buzzer_on_off),
            "enableBuzzerConfirmation":active(data_buzzer_confirm_on_off),
    ...
    function sigfoxProductConfigurationN2DataOutput(stringHex)
    {
        ...
        data={
            ...
            "periodDatalogTransmission":transmissionPeriodHistorical(data_datalog_tx_period),
            "pendingJoin":pendingJoin(data_pending_join),
            "dateYear": calibrationActivation(data_date_produit_annee),
            "dateMonth":co2Threshold(data_date_produit_mois),
            "dateDay":co2Threshold(data_date_produit_jour),
            "dateHour":active(data_date_produit_heure),
            "dateMinute":active(data_date_produit_minute),
        }
        return data
    }
} // end of decoder
